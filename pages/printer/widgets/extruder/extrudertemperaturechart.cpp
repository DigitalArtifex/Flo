...
ExtruderTemperatureChart::~ExtruderTemperatureChart()
{
    // if(m_axisX)
    //     m_axisX->deleteLater();
...
void ExtruderTemperatureChart::setupUi()
{
    m_chart = new QChart();
...
void ExtruderTemperatureChart::onUpdateTimerTimeout()
{
    QDateTime timestamp = QDateTime::currentDateTime();
    ...
    //Update bed temps
    QString key = "Bed";
    qreal temperature = m_instance->currentTemp();

    key = m_instance->name();
    temperature = m_instance->currentTemp();
...
void ExtruderTemperatureChart::setStyleSheet(QString &styleSheet)
{
    QLinearGradient plotAreaGradient;
    plotAreaGradient.setStart(QPointF(0, 1));
    plotAreaGradient.setFinalStop(QPointF(1, 0));
